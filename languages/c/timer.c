...
/**Initialise a timer structure to expire in the future
 * @param period, time in milliseconds for when to expire, the timer will expire
 * this many milliseconds in the future
 * @return timer structure, this can be used to initialise a timer structure by
 * return a structure - not a pointer to it.*/
util_timer_t util_timer_start_ms(time_ms_t period);

/**@brief check whether a timer has expired
 * @param t, timer structure
 * @return true if the timer has expired, false otherwise, this function will
 * continue to return true until the timer is reset */
bool util_timer_expired(util_timer_t *t);

/* This is the non-portable bit, to make a proper test bench this function
 * could be replaced with a series of 'tricky' values to test that could
 * be returned */
time_ms_t util_time_ms(void) /**@bug This really should be a monotonic clock source, being lazy here*/
{
#include <sys/time.h>
  struct timeval tp;
...
{
  assertion(t && t->initialized);
  return util_timer_expired(t) && !(t->fired_once); /**@warning The order of execution of this statement matters*/
}
...
bool util_timer_expired_cyclic(util_timer_t *t)
{
  ...
  {
    const time_ms_t expires = (t->start + t->period);
    time_ms_t jitter = MAX(t->expired_on, expires) - MIN(t->expired_on, expires);
    t->expired = false;
    t->expired_on = 0;
    t->fired_once = false;
    jitter %= t->period;
...
int main(void)
{
	...
	fprintf(stdout, "timer fired (%lu %lu %s)\n", (unsigned long)start, (unsigned long)end, end-start>start?"true":"false");
#endif
	for(;;) {
